...
#include <bigsaddle/gui/gui_user.h>
...
void ExampleApp::ShowExampleDialog()
{
	char temp[1024];
	bx::snprintf(temp, BX_COUNTOF(temp), "Example: %s", getName() );
	...
	if (s_showStats)
	{
		...
		if (ImGui::Begin(ICON_FA_BAR_CHART " Stats", &s_showStats) )
		{
			if (ImGui::CollapsingHeader(ICON_FA_PUZZLE_PIECE " Resources") )
			{
				...
				ImGui::PushFont(GuiFont::Kind::Mono);
